...
async function getAnalytics(adminToken) {
    try {
        const response = await fetch('http://localhost:3000/api/orders/admin/analytics', {
            headers: {
                'Authorization': `Bearer ${adminToken}`
            }
        });
...
async function getProducts(adminToken) {
    try {
        const response = await fetch('http://localhost:3000/api/products/admin/all?include_inactive=true', {
            headers: {
                'Authorization': `Bearer ${adminToken}`
            }
        });
...
async function onActiveChange(productId) {
    try {
        ...
        const response = await fetch(`http://localhost:3000/api/products/${productId}/flip-status`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${adminToken}`
...
async function editImages() {
    try {
        ...
        const deleteResponse = await fetch(`http://localhost:3000/api/products/${productId}/images`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${adminToken}`
